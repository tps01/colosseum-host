...
def verify_maximum(
    *,
    ...
    return VerificationResult(
        status="FAIL",
        message=f"expected <= {maximum}{unit_suffix}, got {actual}",
        optional=optional,
        actual=actual,
    )


def maximum_verifier(
    command: str,
    *,
    name: str,
    unit: str = "",
    ...
    verify.__module__ = str(caller_module)
    verify.__doc__ = (
        f"Require a prior ``{source_command}`` measurement to be at most a maximum value.\n\n"
        f":param key: Measurement key shared with the prior ``{source_command}`` call.\n"
        ":type key: str\n"
